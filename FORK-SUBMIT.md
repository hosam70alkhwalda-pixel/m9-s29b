# Module 9 Week B — Stretch Thu — Fork-and-Submit Flow
...
3. **Clone your fork** (not this repo):
   `git clone https://github.com/<you>/m9-s29b.git`
4. **Branch, implement, commit, push:**
   ```bash
   git checkout -b stretch-9b-thu-graphrag
   # edit the files (see the Stretch Thu page for the deliverables)
   git add .
   git commit -m "stretch-9b-thu: implementation"
   git push origin stretch-9b-thu-graphrag
   ```
5. **Open a PR *within your fork*** — base `main`, compare `stretch-9b-thu-graphrag`,
   **base repository = your fork** (GitHub defaults to upstream — change it).
   When CI passes, paste the PR URL into
   TalentLMS → Module 9 Week B → Stretch Thu.
...
- **Slow first run.** The first `load_fixture.py` invocation downloads the
  `all-MiniLM-L6-v2` model (~90 MB) and embeds every recipe description.
  Expect 1–2 minutes. Subsequent runs reuse the HuggingFace cache.
